...
def get_random_youtube_hash():
    ...
    # youtube-dl --yes-playlist --get-id
    # https://www.youtube.com/watch\?v\=Hurz0ZQbbMU\&list\=UU-lHJZR3Gqxm24_Vd_AJ5Yw
    # > valid_hashes.txt
...
def sao(episode=0):
    ...
    # youtube-dl --yes-playlist --get-id
    # https://www.youtube.com/playlist\?list\=PLvTFVgN7IAZ5i28K75ocPWALV0_Gpv0Q1
    # > sao.txt
...
def login():
    if request.method == "GET":
        ...
        if userdb.verify(request.form['username_in'], request.form['password_in']):
            ...
            session['username_hash'] = userdb.username_hash(
                request.form['username_in'])
            session['password_hash'] = userdb.password_hash(
